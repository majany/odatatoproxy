/**
 * Custom UI5 Server middleware for forwarding odata requests to connectUI5proxy middleware
 *
...
module.exports = function ({ resources, options }) {
	...
	// one time setup. take odata path from manifest or middleware config
	...
	return async function (req, res, next) {
		...
		if (sUri && req.url.startsWith(sUri) && proxy) {
			// matches our odata services so we redirect to our proxy
